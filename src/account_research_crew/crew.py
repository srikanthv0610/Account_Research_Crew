...
class ResearchCrew:
    ...
    @agent
    def executive_briefing_agent(self) -> Agent:
        """
        Create the executive briefing agent.

        This agent takes the insights generated by the account insight agent
        and formats them into a structured briefing document suitable for sales meetings.
        """
        return Agent(
            config=self.agents_config["executive_briefing_agent"],
            llm=llm,
            verbose=True,
        )
    ...
    def executive_briefing_task(self) -> Task:
        """
        Define the executive briefing task that compiles insights into a final report.

        This task takes the output of the account insight task and formats it into
        a structured briefing document suitable for sales meetings.
        """
        
        return Task(
        config=self.tasks_config["executive_briefing_task"],
        agent=self.executive_briefing_agent(),
        context=[self.company_research_task(), self.account_insight_task()],
        )
